...
class EventPost {
    ...
    function load_styles() {
        //CSS
        wp_register_style('eventpost', plugins_url('/css/eventpost.css', __FILE__), false,  $this->version);
        wp_enqueue_style('eventpost', plugins_url('/css/eventpost.css', __FILE__), false,  $this->version);
    ...
    function print_date($post = null, $links = 'deprecated') {
        ...
        if ($event->start != '' && $event->end != '') {
            ...
            if (date('d/m/Y', $event->time_start) == date('d/m/Y', $event->time_end)) {
                if (date('H:i', $event->time_start) != date('H:i', $event->time_end) && date('H:i', $event->time_start) != '00:00' && date('H:i', $event->time_end) != '00:00') {
                    $dates.='<time class="time wday" itemprop="dtstart" datetime="' . date('c', $event->time_start) . '">' . date('g:ia', $event->time_start) . ' - </time><time class="time wday" itemprop="dtend" datetime="' . date('c', $event->time_end) . '">' . date('g:ia', $event->time_end) . '</time>';
                } elseif (date('H:i', $event->time_start) != '00:00') {
                    $dates.='<span class="linking_word">,' . __('at:', 'eventpost') . '</span>
					<time class="time wday" itemprop="dtstart" datetime="' . date('c', $event->time_start) . '">' . date('H:i', $event->time_start) . '</time>';
                }
                $dates.= ' <span class="wday">' . $this->human_date($event->time_end) . "</span>";
            } else {
                $dates.= '
		...
		';
            ...
            if (!is_admin() && $event->time_end > time() && (
                ...
                $dates.='
		<br/><a href="' . $ics_url . '" class="event_link ics" target="_blank" title="' . __('Download ICS file', 'eventpost') . '">iCal</a>
